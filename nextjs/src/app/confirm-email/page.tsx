...
import { ConfirmationStateRoute } from "@/fastify/src/routes/auth/confirmation-state.routes";
import axios from "axios";
import Image from "next/image";
import Link from "next/link";
import { redirect } from "next/navigation";
import ConfirmationForm from "./confirmation-form";
import ResendText from "./resend-text";
...
export default async function ConfirmEmailPage({
  ...
  const { uuid, code } = searchParams;
  if (!uuid) {
    redirect("/");
  }

  const { state }: StateObject = await axios
    .get<ConfirmationStateRoute["Reply"]["2xx"]>("/auth/confirmation-state", {
      params: {
        uuid: uuid,
      },
      ...
      <div className="px-8 py-4 absolute top-0 left-0 flex w-full h-full justify-center items-center">
        <div className="w-full flex flex-col items-center">
          ...
          <div className="bg-white px-8 py-8 rounded-xl w-3/4 max-w-[720px] flex flex-col items-center shadow">
            ...
            <ConfirmationForm uuid={uuid} defaultValue={code} />
            <Separator className="mt-2 mb-5 max-w-[600px]" />
            <ResendText uuid={uuid} />
          </div>
        </div>
      </div>
    </main>
